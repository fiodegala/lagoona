...
import { Search, Share2, Loader2, ChevronLeft, ChevronRight, Download, X } from 'lucide-react';
...
const ResellerCatalogPage = () => {
  ...
  const loadVariations = async (activeProducts: Product[]) => {
    try {
      ...
      if (productIds.length === 0) { setVariationsLoading(false); return; }
      ...
      if (!variations || variations.length === 0) { setVariationsLoading(false); return; }
      ...
      const varChunks = [];
      for (let i = 0; i < varIds.length; i += chunkSize) varChunks.push(varIds.slice(i, i + chunkSize));
      const varValueResults = await Promise.all(
        varChunks.map((chunk) =>
          supabase.from('product_variation_values').select('variation_id, attribute_value_id').in('variation_id', chunk)
        )
      );
      varValueResults.forEach((r) => { if (r.data) allVarValues = allVarValues.concat(r.data); });

      const allAttrValueIds = [...new Set(allVarValues.map((vv) => vv.attribute_value_id))];
      const attrChunks = [];
      for (let i = 0; i < allAttrValueIds.length; i += chunkSize) attrChunks.push(allAttrValueIds.slice(i, i + chunkSize));
      let allAttrValues: { id: string; value: string; attribute_id: string }[] = [];
      const attrValueResults = await Promise.all(
        attrChunks.map((chunk) =>
          supabase.from('product_attribute_values').select('id, value, attribute_id').in('id', chunk)
        )
      );
      attrValueResults.forEach((r) => { if (r.data) allAttrValues = allAttrValues.concat(r.data); });

      const allAttrIds = [...new Set(allAttrValues.map((av) => av.attribute_id))];
      const attrDefChunks = [];
      for (let i = 0; i < allAttrIds.length; i += chunkSize) attrDefChunks.push(allAttrIds.slice(i, i + chunkSize));
      let colorAttrIdSet = new Set<string>();
      const attrDefResults = await Promise.all(
        attrDefChunks.map((chunk) =>
          supabase.from('product_attributes').select('id, name').in('id', chunk).ilike('name', '%cor%')
        )
      );
      attrDefResults.forEach((r) => { if (r.data) r.data.forEach((a) => colorAttrIdSet.add(a.id)); });

      const colorAttrValuesMap: Record<string, string> = {};
      allAttrValues.forEach((av) => {
        if (colorAttrIdSet.has(av.attribute_id)) colorAttrValuesMap[av.id] = av.value;
      });
      ...
      variations.forEach((v) => {
        ...
        if (colorLabel && map[v.product_id].some((existing) => existing.label === colorLabel)) {
          ...
          if (!map[v.product_id][existingIdx].image_url && v.image_url) map[v.product_id][existingIdx].image_url = v.image_url;
          return;
        }
        ...
        map[v.product_id].push({ id: v.id, image_url: v.image_url, label });
      });
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore) setVisibleCount((prev) => Math.min(prev + PAGE_SIZE, filtered.length));
      },
      { rootMargin: '400px' }
    );
  ...
  const catalogUrl = typeof window !== 'undefined' ? window.location.origin + '/catalogo-revendedor' : '';

  const shareCatalog = () => {
    const text = encodeURIComponent(`Confira nosso catálogo para revendedores:\n${catalogUrl}`);
  ...
  const generatePDF = useCallback(async () => {
    ...
    try {
      ...
      doc.text('Catálogo para Revendedores', 105, 20, { align: 'center' });
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(120, 120, 120);
      doc.text('Solicite valores pelo WhatsApp', 105, 27, { align: 'center' });
      ...
      categoryMap.forEach((prods, catName) => {
        ...
        const tableData = prods.map((p) => {
          ...
          return [p.name, colors || '—'];
        });

        autoTable(doc, {
          startY,
          head: [['Produto', 'Cores Disponíveis']],
          body: tableData,
      ...
      doc.save('catalogo-revendedor-fio-de-gala.pdf');
    } catch (e) {
  ...
  const askAboutProduct = (product: Product) => {
    ...
    const text = encodeURIComponent(
      `Olá! Sou revendedor(a) e tenho interesse no produto: *${product.name}*\nGostaria de saber os valores para revenda.\n${productUrl}`
    );
  ...
  const productImagesMap = useMemo(() => {
    ...
    products.forEach((product) => {
      ...
      variations.forEach((v) => { if (v.image_url && !images.includes(v.image_url)) images.push(v.image_url); });
  ...
  const openLightbox = useCallback((productId: string, index: number) => setLightbox({ productId, index }), []);
  ...
  const lightboxNavigate = useCallback((direction: 'prev' | 'next') => {
    ...
    const next = direction === 'next' ? (lightbox.index + 1) % images.length : (lightbox.index - 1 + images.length) % images.length;
        ...
        <div className="bg-primary/5 py-8 md:py-12">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-muted-foreground text-sm md:text-base mb-4">
              Confira todos os nossos produtos — solicite valores pelo WhatsApp
            </p>
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {visibleProducts.map((product, productIndex) => {
                  const variations = variationsMap[product.id] || [];
                  return (
                    <div key={product.id} className="group rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden flex flex-col">
                      {/* Image */}
                      {(() => {
                        const images = productImagesMap[product.id] || ['/placeholder.svg'];
                        ...
                        return (
                          <div
                            ...
                            onTouchEnd={(e) => {
                              ...
                              if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 40) navigateImage(product.id, dx < 0 ? 'next' : 'prev');
                            }}
                          >
                            <img
                            ...
                            {hasMultiple && (
                              <>
                                ...
                                <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-1">
                                  {images.map((_, i) => (
                                    <span key={i} className={`block w-1.5 h-1.5 rounded-full transition-colors ${i === currentIdx ? 'bg-primary' : 'bg-background/60'}`} />
                                  ))}
                                </div>
                              </>
                            )}
                        ...
                        <div className="px-3 pt-2 flex flex-wrap gap-1.5">
                          {variations.map((v) => {
                            ...
                            return (
                              <button
                                key={v.id}
                                onClick={() => { if (imgIdx >= 0) setImageIndex((prev) => ({ ...prev, [product.id]: imgIdx })); }}
          ...
          return (
            <div
              ...
              onTouchEnd={(e) => {
                ...
                const dx = e.changedTouches[0].clientX - startX;
                if (Math.abs(dx) > 50) { e.stopPropagation(); lightboxNavigate(dx < 0 ? 'next' : 'prev'); }
              }}
            >
              <button onClick={closeLightbox} className="absolute top-4 right-4 z-10 bg-background/20 backdrop-blur-sm rounded-full p-2 text-white hover:bg-background/40 transition-colors">
                <X className="h-6 w-6" />
              </button>
              {images.length > 1 && (
              ...
              {images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); lightboxNavigate('prev'); }} className="absolute left-3 top-1/2 -translate-y-1/2 z-10 bg-background/20 backdrop-blur-sm rounded-full p-2 text-white hover:bg-background/40 transition-colors">
                    <ChevronLeft className="h-6 w-6" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); lightboxNavigate('next'); }} className="absolute right-3 top-1/2 -translate-y-1/2 z-10 bg-background/20 backdrop-blur-sm rounded-full p-2 text-white hover:bg-background/40 transition-colors">
