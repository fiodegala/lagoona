      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Crie informativos e pop-ups para os usuários do sistema</p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card><CardContent className="py-12 text-center text-muted-foreground">Nenhum comunicado criado.</CardContent></Card>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {a.is_active ? <Badge className="bg-green-100 text-green-700">Ativo</Badge> : <Badge variant="secondary">Inativo</Badge>}
                        <Badge variant="outline">{a.target_type === 'all' ? 'Todos' : `${(a.target_user_ids || []).length} usuários`}</Badge>
                      </div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {a.expires_at && <> • Expira em {format(new Date(a.expires_at), 'dd/MM/yyyy HH:mm')}</>}
                      </p>
                    </div>
                    <div className="flex gap-1">
                      <Button size="icon" variant="ghost" onClick={() => openEdit(a)}><Edit className="h-4 w-4" /></Button>
                      <Button size="icon" variant="ghost" className="text-destructive" onClick={() => { if (confirm('Excluir este comunicado?')) deleteMutation.mutate(a.id); }}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                </CardContent>
              </Card>
